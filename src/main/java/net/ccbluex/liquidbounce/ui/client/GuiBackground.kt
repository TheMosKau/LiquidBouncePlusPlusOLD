...
class GuiBackground(val prevGui: GuiScreen) : GuiScreen() {
    ...
    override fun initGui() {
        enabledButton = GuiButton(1, width / 2 - 100, height / 4 + 35, "Shader Background: (${if (enabled) "ON" else "OFF"})")
        buttonList.add(enabledButton)
        particlesButton = GuiButton(2, width / 2 - 100, height / 4 + 50 + 25, "Particle: (${if (particles) "ON" else "OFF"})")
